...
class AudioProfiles(AppletPlugin):
    ...
    def on_load(self) -> None:
        ...
        self._dbus_add_method("UpdateLastProfile", ("s", "s"), "", self.update_last_profile)
    ...
    def add_device_profile_menu(self, device: "Device") -> None:
        ...
        def _generate_profiles_menu(info: "CardInfo") -> List["SubmenuItemDict"]:
            ...
            for profile in info["profiles"]:
                ...
                if profile["name"] == self.self.last_selected_audio_profile[path] and profile["name"] != info["active_profile"]:
                    self.on_activate_profile(device, profile)
                items.append({
                    "text": profile_name,
                    "markup": True,
    ...
    def on_device_property_changed(self, path: str, key: str, value: Any) -> None:
        if key == "Connected":
            self.clear_menu()
            if path not in self.self.last_selected_audio_profile:
